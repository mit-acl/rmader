...
if __name__ == '__main__':
    ...
    # initialization
    home_dir = "/media/kota/T7/rmader_ral2"
    num_of_agents = 10
    methods = ["edg_team"]
    ...
    for method in methods: 
        ...
        for cd in cd_list:
            ...
            if method == "oldmader":
                # old mader
                source_dir = f"/media/kota/T7/rmader_ral2/oldmader/bags/cd{cd}ms"
                first_agent_idx = 1
                dt = 0.01 #[s] MADERm EGO-Swarm, EDG-Team's state/pos_cmd is published every 10ms
            elif method == "rmader":
                # rmader
                source_dir = f"/media/kota/T7/rmader_ral2/rmader/bags/cd{cd}ms/dc{dc}ms"
                first_agent_idx = 1
                dt = 0.01 #[s] MADERm EGO-Swarm, EDG-Team's state/pos_cmd is published every 10ms
            elif method == "wo_check_rmader":
                # rmader without check
                source_dir = f"/media/kota/T7/rmader_ral2/wo_check_rmader/bags/cd{cd}ms/dc{dc}ms"
                first_agent_idx = 1
                dt = 0.01 #[s] MADERm EGO-Swarm, EDG-Team's state/pos_cmd is published every 10ms
            elif method == "ego_swarm":
                # EGO-Swarm
                source_dir = f"/media/kota/T7/rmader_ral2/ego_swarm/bags/cd{cd}ms"
                first_agent_idx = 0
                dt = 0.01 #[s] MADERm EGO-Swarm, EDG-Team's state/pos_cmd is published every 10ms
            elif method == "edg_team":
                # EDG-Team
                source_dir = f"/media/kota/T7/rmader_ral2/edg_team/bags/cd{cd}ms"
                first_agent_idx = 0
                dt = 0.01 #[s] MADERm EGO-Swarm, EDG-Team's state/pos_cmd is published every 10ms
            ...
            for rosbag_name in rosbags:
                ...
                if method == "rmader" or method == "wo_check_rmader" or method == "oldmader":
                    ...
                    sim_id = rosbag_name[source_len+14:source_len+17]
                ...
                if method == "rmader" or method == "wo_check_rmader" or method == "oldmader":
                    ...
                    log_data = b.message_by_topic("/drone_0_visual_slam/odom")
                        ...
                        for ii in range(len(times_discret)-1):
                            ...
                            try:
                                ...
                                if (dist < bbox_limit):
                                    ...
                                    print(agent1, agent2, "dist: ", dist)
                                    os.system(f'echo "simulation '+sim_id+': ***collision*** at '+str(time)+' with '+str(agent1)+' and '+str(agent2)+'" >> '+source_dir+'/collision_status.txt')
