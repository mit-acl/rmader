...
tmux send-keys -t $SESSION:$w.2 "roslaunch --wait rmader hw_obstacle.launch veh:=SQ num:=01 x:=-3 y:=3" C-m 
tmux send-keys -t $SESSION:$w.4 "roslaunch --wait rmader onboard.launch veh:=SQ num:=02 x:=0 y:=3" C-m
...
tmux send-keys -t $SESSION:$w.10 "roslaunch --wait rmader onboard.launch veh:=SQ num:=05 x:=0 y:=-3" C-m
...
if [[ $snapsim_or_perfect == "snap_sim" ]]; then
	...
	tmux send-keys -t $SESSION:$w.5 "roslaunch --wait snap_sim sim.launch veh:=SQ num:=02 x:=0 y:=3" C-m
	...
	tmux send-keys -t $SESSION:$w.11 "roslaunch --wait snap_sim sim.launch veh:=SQ num:=05 x:=0 y:=-3" C-m
	...
	tmux send-keys -t $SESSION:$w.5 "roslaunch --wait rmader perfect_tracker_and_sim.launch quad:=SQ02s x:=0 y:=3" C-m
	...
	tmux send-keys -t $SESSION:$w.11 "roslaunch --wait rmader perfect_tracker_and_sim.launch quad:=SQ05s x:=0 y:=-3" C-m
