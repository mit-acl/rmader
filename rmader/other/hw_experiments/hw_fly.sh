...
tmux send-keys -t $SESSION:$w.4 "ssh root@nx03.local" C-m  
tmux send-keys -t $SESSION:$w.6 "ssh root@nx04.local" C-m
tmux send-keys -t $SESSION:$w.8 "ssh root@nx05.local" C-m
...
tmux send-keys -t $SESSION:$w.11 "ssh nuc08@192.168.23.2" C-m
...
if [[ $ifDELETE == 'true' ]]; then 
	...
	tmux send-keys -t $SESSION:$w.5 "cd /home/nuc3/Research/bags && rm *.txt && cd" C-m
	tmux send-keys -t $SESSION:$w.7 "cd /home/nuc4/Research/bags && rm *.txt && cd" C-m
	tmux send-keys -t $SESSION:$w.9 "cd /home/nuc5/Research/bags && rm *.txt && cd" C-m
...
# tmux send-keys -t $SESSION:$w.5 "(roscd rmader && git rev-parse HEAD && git diff --color && cd /home/nuc3/Research/bags/ && roslaunch rmader hw_onboard.launch quad:=NX03) 2>&1 | tee ~/Research/bags/nx03_rmader_$(date '+%Y_%m_%d_%H_%M_%S').txt" C-m
# tmux send-keys -t $SESSION:$w.7 "(roscd rmader && git rev-parse HEAD && git diff --color && cd /home/nuc4/Research/bags/ && roslaunch rmader hw_onboard.launch quad:=NX04) 2>&1 | tee ~/Research/bags/nx04_rmader_$(date '+%Y_%m_%d_%H_%M_%S').txt" C-m
# tmux send-keys -t $SESSION:$w.9 "(roscd rmader && git rev-parse HEAD && git diff --color && cd /home/nuc5/Research/bags/ && roslaunch rmader hw_onboard.launch quad:=NX05) 2>&1 | tee ~/Research/bags/nx05_rmader_$(date '+%Y_%m_%d_%H_%M_%S').txt" C-m
...
tmux send-keys -t $SESSION:$w.1 "roslaunch rmader onboard.launch quad:=NX01" C-m
tmux send-keys -t $SESSION:$w.3 "roslaunch rmader onboard.launch quad:=NX02" C-m
tmux send-keys -t $SESSION:$w.5 "roslaunch rmader onboard.launch quad:=NX03" C-m
tmux send-keys -t $SESSION:$w.7 "roslaunch rmader onboard.launch quad:=NX04" C-m
tmux send-keys -t $SESSION:$w.9 "roslaunch rmader onboard.launch quad:=NX05" C-m
tmux send-keys -t $SESSION:$w.11 "roslaunch rmader onboard.launch quad:=NX08" C-m
