...
tmux send-keys -t $SESSION:$w.10 "ssh root@nx04.local" C-m  
...
tmux send-keys -t $SESSION:$w.11 "ssh nuc4@192.168.18.2" C-m
...
if [[ $ifDELETE == 'true' ]]; then 
	...
	tmux send-keys -t $SESSION:$w.11 "cd /home/nuc4/Research/bags && rm *.txt && cd" C-m
...
tmux send-keys -t $SESSION:$w.11 "(roscd rmader && git rev-parse HEAD && git diff --color && cd /home/nuc4/Research/bags/ && roslaunch rmader hw_onboard.launch quad:=NX04) 2>&1 | tee ~/Research/bags/nx04_rmader_$(date '+%Y_%m_%d_%H_%M_%S').txt" C-m
