...
int main(int argc, char** argv)
{
  ...
  // Dynmaic obstacles
  // ConvexHullsOfCurve hulls_curve = createDynamicObstacle(ma_vector, 0.0, 0.0, bbox_z / 2.0, num_pol, bbox_x, bbox_y,
  // bbox_z, t_min, t_max);
  ...
  // publish the initial position and goal

  geometry_msgs::PointStamped initial;
  initial.header.frame_id = "world";
  initial.point = mu::eigen2point(q0);
  ros::Publisher pub_initial = nh.advertise<geometry_msgs::PointStamped>("/oct_initial", 1, true);
  pub_initial.publish(initial);

  geometry_msgs::PointStamped quadgoal;
  quadgoal.header.frame_id = "world";
  quadgoal.point = mu::eigen2point(goal);
