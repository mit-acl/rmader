...
if __name__ == '__main__':
    ...
    os.system("sed -i '/visual/s/^/#/g' $(rospack find rmader)/param/rmader.yaml") #comment out visual param
    os.system("sed -i '/basis/s/^/#/g' $(rospack find rmader)/param/rmader.yaml") #comment out basis param
    ...
    for k in range(len(total_num_of_obs)):
        for j in range(len(all_basis)):
            ...
            for s in range(num_of_sims):
                ...
                commands.append("roslaunch rmader rmader_specific_and_general.launch gui:=false rviz:=false environment:=false");
                commands.append("sleep 1.0 &&rosrun rmader dynamic_corridor.py "+str(total_num_of_obs[k]));
                commands.append("sleep 3.0 && rosparam set /SQ01s/rmader/basis "+basis); #Remember to comment the parameter "basis" in rmader.yaml before running this file
                commands.append("sleep 3.0 && rosparam set /SQ01s/rmader/visual false"); #Remember to comment the parameter "visual" in rmader.yaml before running this file

                commands.append("sleep 5.0 && roslaunch rmader rmader.launch");
    ...
    time.sleep(3.0)
    os.system("sed -i '/visual/s/^#//g' $(rospack find rmader)/param/rmader.yaml") #uncomment visual param
    os.system("sed -i '/basis/s/^#//g' $(rospack find rmader)/param/rmader.yaml") #uncomment basis param
